...
export default function useQRCodeLogoSize({
  ...
  useEffect(() => {
    if (logoHref) {
      ...
      if (typeof logoHref === 'number') {
        ...
        transformLogoSize(width, isRounded ? width : height);
      }

      // if logo is string & remote
      if (typeof logoHref === 'string') {
        Image.getSize(logoHref, (width, height) => {
            transformLogoSize(width, isRounded ? width : height);
          }, () =>
      ...
      if (
        ...
        Image.getSize(logoHref.uri, (width, height) => {
            transformLogoSize(width, isRounded ? width : height);
          }, () =>
  ...
  }, [logoHref, transformLogoSize, isRounded]);
